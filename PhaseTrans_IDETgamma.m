		...
		for sparsity = ceil(rho*nummeasurements)
			...
			for ii=1:numtrials
                ...
                if ~IDETgamma7fail
					fprintf('IDETgamma7 ... \t');
					dstart = tic;
                    xhat                         = IDETgamma(Phi, y, 0.7, 'Tolerance',tolerance);
	                % 记录第1个算法结果
                 ...
                 if ~IDETgamma8fail
					fprintf('IDETgamma8 ... \t');
					dstart = tic;
                    xhat                         = IDETgamma(Phi, y, 0.8, 'Tolerance',tolerance);
                    % 记录第2个算法结果
                 ...
                 if ~IDETgamma9fail
					fprintf('IDETgamma9 ... \t');
					dstart = tic;
                    xhat                         = IDETgamma(Phi, y, 0.9, 'Tolerance',tolerance);
                    % 记录第3个算法结果
...
tt = datevec(now);
str = num2str(tt(6));
filename = strcat('PhaseTrans_IDETgamma', '_', str, '.mat'); 
save(filename)
...
ylabel('Probability of Exact Recovery');
grid on;
legend('IDET-\gamma=0.7', 'IDET-\gamma=0.8', 'IDET-\gamma=0.9');
% fn = strcat('Fig_PhaseTrans_IDETgamma',  '.fig'); 
% saveas(gcf, fn) 
set(gcf,'color','none');
set(gca,'color','none');
set(gcf,'InvertHardCopy','off');
print(gcf,'-depsc2',['Fig_PhaseTrans_IDETgamma_recovery' '.eps']);
...
set(gca,'color','none');
set(gcf,'InvertHardCopy','off');
print(gcf,'-depsc2',['Fig_PhaseTrans_IDETgamma' '.eps']);
